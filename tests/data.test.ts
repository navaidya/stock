...
describe('watchlist and universe data files', () => {
  ...
  it('[COL-8] only reference declared segments', () => {
...
describe('hydrate', () => {
  it('[MOD-9] keeps a row visible when no data was collected for it', () => {
  ...
  it('[MOD-8] merges collected metrics onto the curated entry', () => {
...
describe('freshness', () => {
  it('[MOD-10] flags data that has never been collected', () => {
    expect(freshness('').stale).toBe(true);
  });

  it('[MOD-10] flags unparseable timestamps rather than showing NaN', () => {
    expect(freshness('not a date').stale).toBe(true);
  });

  it('[MOD-10] treats data older than a day as stale', () => {
  ...
  it('[MOD-10] does not flag a recent refresh', () => {
...
describe('payingDividend', () => {
  it('[UI-5] excludes non-payers and missing data', () => {
